...
const AppRoutes = () => {
  ...
  return (
    <Suspense fallback={<Bullseye><Spinner /></Bullseye>}>
      <Routes>
        <Route path="/" element={isAuthenticated ? (<ChatbotMain />) : (<Navigate to="/login" replace />)} />
        <Route path="/login" element={!isAuthenticated ? (<LoginWithOpenShift setIsAuthenticated={setIsAuthenticated} />) : (<Navigate to="/" replace />)} />
